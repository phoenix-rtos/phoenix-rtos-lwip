 ...
 * Required frequencies are set in PLO. 
 * 
...
static int enet_initClocks(enet_state_t *state)
{
	...
	volatile struct MCU_PLL0_CFG *pll_cfg;

	uint8_t pll_num[] = {clk_mcu_r5fss_pll0, clk_mcu_cpsw_pll2, clk_mcu_cpsw_pll2};
	uint8_t hsdiv_num[] = {0, 0, 1};
	uint64_t expected_clk[] = {1000000000, 250000000, 500000000};

	/* TODO: check if PLL active (plo activates all PLLs)* /

	/* check PLL,HSDIV frequencies */
	pctl.action = pctl_get;
	pctl.type = pctl_frequency;

    for (int i=0; i<3; i++) {
		pctl.frequency.pll_num = pll_num[i];
		pctl.frequency.hsdiv = hsdiv_num[i];
		platformctl(&pctl);

		if (pctl.frequency.val > expected_clk[i] + 1 || pctl.frequency.val < expected_clk[i] - 1) {
			enet_debug("clock value PLL%d HSDIV%d is %lu and exceeds expected %lu", pctl.frequency.pll_num, pctl.frequency.hsdiv, pctl.frequency.val, expected_clk[i]);
			return -EINVAL;
		}
	}

	/* configure MUX */
	state->mmr0_ctrl->CTRLMMR_MCU_ENET_CLKSEL &= ~(0xF << 8);
	state->mmr0_ctrl->CTRLMMR_MCU_ENET_CLKSEL |= (ENET_MCU_PLL2_HSDIV1_CLKOUT << 8);
...
static void enet_setLinkState(void *arg, linkstate_t status, uint16_t link_speed)
{
	...
	enet_debug("link is %s, %uMbps in %s duplex", status.LINK_STATUS? "UP" : "DOWN", link_speed, 
