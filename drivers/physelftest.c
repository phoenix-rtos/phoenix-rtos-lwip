...
static err_t physelftest_netifInput(struct pbuf *p, struct netif *netif)
{
	...
	const size_t expected_len = TEST_PACKET_LEN + ETH_PAD_SIZE + (test_common.params->is_crc_stripped ? 0 : CRC_LEN);
...
int physelftest(const struct selftest_params *params)
{
	...
	int err = params->setup(params->arg);
	if (err < 0) {
		return err;
	}
	...
	err = params->teardown(params->arg);
	if (err < 0) {
		ret = err;
	}
