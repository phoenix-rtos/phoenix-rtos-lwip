...
/* DP8386IS-specific registers*/
enum {
	EPHY_DP83867IS_0_BMCR = 0x00,        /* Basic Mode Control Register */
	EPHY_DP83867IS_0D_REGCR = 0x0D,      /* MMD Access Control */
	EPHY_DP83867IS_0E_ADDAR = 0x0E,      /* MMD Access Address/Data */
	...
	EPHY_DP83867IS_D3_SGMIICTL1 = 0x00D3 /* SGMIICTL1 (via MMD access) */
};
...
static __attribute__((unused)) inline uint16_t ephy_mmdRead(const eth_phy_state_t *phy, uint16_t devad, uint16_t addr)
{
	ephy_regWrite(phy, EPHY_MMD_0D_MACR, devad);
	ephy_regWrite(phy, EPHY_MMD_0E_MAADR, addr);
	ephy_regWrite(phy, EPHY_MMD_0D_MACR, (1 << 14) | devad);
...
{
	uint32_t devad = 0x1Fu;
	// Enable differential SGMII clock to MAC
	ephy_mmdWrite(phy, devad, 0xD3, 0x4000);

	// Enable SGMII autonegotiation and speed optimization
	ephy_regWrite(phy, EPHY_DP83867IS_0_BMCR, 0x1000u);
	ephy_regWrite(phy, EPHY_DP83867IS_14_SGMIICR, 0x2BC0u);

	// Set SGMII Auto-Negotiation Timer Duration to 11 ms
	ephy_mmdWrite(phy, devad, 0x31, 0x0070);

	// Disable RGMII
	ephy_mmdWrite(phy, devad, 0x32, 0x0);

#ifdef EPHY_DEBUG
	uint32_t reg;
	reg = ephy_mmdRead(phy, devad, 0xD3);
	ephy_debug_printf(phy, "extended reg 0xD3: %08x", reg);
	reg = ephy_mmdRead(phy, devad, 0x31);
	ephy_debug_printf(phy, "extended reg 0x31: %08x", reg);
	reg = ephy_mmdRead(phy, devad, 0x32);
	ephy_debug_printf(phy, "extended reg 0x32: %08x", reg);
#endif
}
...
int ephy_init(eth_phy_state_t *phy, char *conf, uint8_t board_rev, link_state_cb_t cb, void *cb_arg)
{
	...
	switch (phy->model) {
		...
		case ephy_ksz9031mnx:
			...
			break;
		case ephy_dp83867is:
			ephy_regWrite(phy, EPHY_DP83867IS_1E_CFG3, 0x82u);
