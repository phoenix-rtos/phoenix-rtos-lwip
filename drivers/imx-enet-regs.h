 ...
 *
 * i.MX RT106x:
 * ENET  base: 0x402D8000  irq 130  (@AIPS-1)  PHY: bus=0, addr=2=; irq=10, reset=9 (GPIO_IO10 and GPIO_IO09)
...
#define ENET_TCR_GTS       BIT(0) /* stop transmit (after current frame transmission ends) */
	...
	uint32_t PALR;
	uint32_t PAUR;                           /* MAC address */
#define ENET_PAUR_TYPE_MASK      BITS(0, 15) /* `Type` field in PAUSE frames */
#define ENET_PAUR_TYPE_RESET_VAL (0x8808)    /* RM 41.5.1.13.4 */
