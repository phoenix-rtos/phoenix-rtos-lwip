...
static void enet_printf(enet_state_t *state, const char *format, ...)
{
	char buf[192];
	va_list arg;
	...
	va_end(arg);
	printf("lwip: enet@%08x %s\n", state->dev_phys_addr, buf);
}
...
static int enet_pinConfig(enet_state_t *state)
{
	...
	const platformctl_t pctl_enet[] = {
		...
		/* 0: GPIO_EMC_25_ALT3, 1: GPIO_B1_10_ALT3 */
		...
		{ pctl_set, pctl_iopad,
			.iopad = { pctl_pad_gpio_b1_10, 0, 2, 0, 0, 0, 3, 6, 1 } }, /* enet1_txclk */
		{ pctl_set, pctl_iopad,
			.iopad = { pctl_pad_gpio_b1_11, 0, 2, 1, 1, 0, 3, 6, 1 } }, /* enet1_rxer */
		{ pctl_set, pctl_iopad,
			.iopad = { pctl_pad_gpio_ad_b0_10, 0, 2, 1, 1, 0, 2, 4, 1 } }, /* irq */
		{ pctl_set, pctl_iopad,
			.iopad = { pctl_pad_gpio_ad_b0_09, 0, 0, 1, 1, 0, 2, 6, 1 } }, /* rst */
...
static int enet_initDevice(enet_state_t *state, int irq, int mdio)
{
	...
	err = enet_reset(state, ENET_RESET_TIMEOUT);
	if (err < 0) {
		return err;
	}
