...
#define OCOTP_SI_REV_OFFSET (0x920)
#define OCOTP_MAC0_OFFSET   (0xA80)
#define OCOTP_MAC1_OFFSET   (0xAA0)
#define OCOTP_MAC2_OFFSET   (0xAC0)
...
static void enet_start(enet_state_t *state)
{
	...
	uint32_t rcr = ENET_RCR_MAX_FL_NO_VLAN_VAL << ENET_RCR_MAX_FL_SHIFT |
			...
			ENET_RCR_PROM |
#endif
#if ENET_INTERNAL_LOOPBACK_MODE && !ENET_RMII_MODE && !ENET_DIS_RX_ON_TX
			ENET_RCR_LOOP |
#endif
			ENET_RCR_MII_MODE;
...
static int enet_readFusedMac(uint32_t *buf)
{
	...
	// FIXME: extract otp memory logic
	buf[0] = va[0x100 + 4 * 0x22];
	buf[1] = va[0x100 + 4 * 0x23];
	buf[2] = va[0x100 + 4 * 0x24];
#elif defined(__CPU_IMXRT117X)
	buf[0] = va[OCOTP_MAC0_OFFSET / sizeof(uint32_t)];
	buf[1] = va[OCOTP_MAC1_OFFSET / sizeof(uint32_t)];
	buf[2] = va[OCOTP_MAC2_OFFSET / sizeof(uint32_t)];
#else
#error "Unsupported TARGET"
#endif
...
static uint32_t enet_readCpuId(void)
{
	...
	res = va[0x100 + 4 * 0x02];
#elif defined(__CPU_IMXRT117X)
	res = va[OCOTP_UID1_OFFSET / sizeof(uint32_t)];
#else
#error "Unsupported TARGET"
#endif
...
static uint8_t enet_readBoardRev(void)
{
	...
	res = va[0x100 + 4 * 0x27];
#elif defined(__CPU_IMXRT117X)
	res = va[OCOTP_SI_REV_OFFSET / sizeof(uint32_t)];
#else
#error "Unsupported TARGET"
#endif
...
static void enet_readCardMac(enet_state_t *state)
{
	...
	else if (state->dev_phys_addr == ENET_ADDR_ENET_1G && enet_readFusedMac(buf) == 0) {
		mac[0] = get_byte(buf[2], 3);
		mac[1] = get_byte(buf[2], 2);
		mac[2] = get_byte(buf[2], 1);
		mac[3] = get_byte(buf[2], 0);
...
static void enet_mdioWait(enet_state_t *state, time_t timeout)
{
#define _MDIO_WAIT_CHECK ((state->mmio->EIR & ENET_IRQ_MII) == 0)
	if (timeout != 0) {
		time_t now, when;
		gettime(&now, NULL);
		when = now + timeout;

		while (_MDIO_WAIT_CHECK) {
			gettime(&now, NULL);
			if (now >= when) {
				enet_printf(state, "enet_mdioWait: WARN: timeout");
				break;
			}
		}
	}
...
static uint16_t enet_mdioIO(enet_state_t *state, unsigned addr, unsigned reg, unsigned val, unsigned op)
{
#if MDIO_DEBUG
	/* clang-format off */
	enet_printf(state, "mdio: op %s, addr=0x%08x, reg=0x%08x, val=0x%04x",
		op == ENET_MMFR_OP_READ_VAL  ? "READ"  :
		op == ENET_MMFR_OP_WRITE_VAL ? "WRITE" :
									  "WARN: undefined",
		addr, reg, val);
	/* clang-format on */
#endif
	...
	if ((addr & NETDEV_MDIO_CLAUSE45) != 0) {
		...
		enet_mdioWait(state, MDIO_TIMEOUT);
...
static int enet_initDevice(enet_state_t *state, int irq, int mdio)
{
	...
	err = interrupt(irq, enet_irq_handler, state, state->irq_cond, &state->irq_handle);
	if (err != 0) {
		enet_printf(state, "Couldn't register interrupt handler: %s (%d)", strerror(err), err);
		return err;
	}
	enet_debug_printf(state, "Interrupt handler initialized successfully (err=%d)", err);

	err = beginthread(enet_irq_thread, 4, state->irq_stack, sizeof(state->irq_stack), state);
	if (err != 0) {
...
static int enet_netifInit(struct netif *netif, char *cfg)
{
	...
	if (cfg != NULL) {
		...
		enet_debug_printf(state, "ephy_init finished, returned %d (%s)", err, strerror(err));
