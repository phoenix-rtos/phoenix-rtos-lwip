...
static int socket_ioctl(int sock, unsigned long request, const void *in_data, void *out_data, size_t size)
{
	...
	case SIOCDELMULTI: {
		...
		case SIOCGIFCONF: {
			...
			if (maxlen != 0) {
				ifreq = (struct ifreq *)((char *)out_data + sizeof(struct ifconf));
		...
		case SIOCDELRT: {
			...
			char *rt_dev = (char *)in_data + sizeof(struct rtentry);
			if (rt_dev == NULL) {
				return -EINVAL;
			}
...
static void do_socket_ioctl(msg_t *msg, int sock)
{
	...
	void *out_data = NULL;
	size_t size;
	const void *in_data = ioctl_unpackEx(msg, &request, NULL, &out_data, &size);

	msg->o.err = socket_ioctl(sock, request, in_data, out_data, size);
}
