...
#define LIST_ENTRY(type) struct { struct type *next, *prev;}

#define LIST_HEAD(type)  struct { struct type *first;}

#define LIST_HEAD_INITIALIZER {NULL}
#define LIST_HEAD_INIT(head) do{(head)->first = NULL;}while(0)
#define LIST_ELEM_INIT(elem, field) do{(elem)->field.next = (elem)->field.prev = elem;}while(0)

#define LIST_ENTRY_INITIALIZER {NULL, NULL}
#define LIST_ENTRY_INIT(elem) do{(elem)->next = (elem)->prev = NULL;}while(0)
...
do { \
    if( ! (h1)->first ){ (h1)->first = (h2)->first; }\
    else if( ! (h2)->first ){ break; }\
    else\
    {\
