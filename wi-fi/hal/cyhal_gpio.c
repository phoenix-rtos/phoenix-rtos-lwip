...
#define CONFIG_WL_PWR_ON_MUX_PAD pctl_mux_lcd_d4
#endif

#ifndef CONFIG_WL_PWR_ON_PAD
#define CONFIG_WL_PWR_ON_PAD pctl_pad_lcd_d4
#endif
...
#define CONFIG_WL_REG_ON_MUX_PAD pctl_mux_csi_d5
#endif

#ifndef CONFIG_WL_REG_ON_PAD
#define CONFIG_WL_REG_ON_PAD pctl_pad_csi_d5
#endif
		...
		bool initVal)
{
	...
	if (CONFIG_WL_PWR_ON_PORT >= 0) {
		...
		if (cyhal_utils_set_iomux(CONFIG_WL_PWR_ON_MUX_PAD, 5) < 0) {
	...
	if (CONFIG_WL_REG_ON_PORT >= 0) {
		...
		if (cyhal_utils_set_iomux(CONFIG_WL_REG_ON_MUX_PAD, 5) < 0) {
			cy_log_msg(CYLF_GPIO, CY_LOG_ERR, "can't configure WL_REG_ON pin iomux\n");
			return CYHAL_GPIO_RSLT_ERR_BAD_PARAM;
		}
		if (cyhal_utils_set_iopad(CONFIG_WL_REG_ON_PAD, 0, 2, 1, 1, 0, 0, 1, 0) < 0) { /* 100K Ohm Pull up */
