...
static int cyhal_sdio_pad_init(void)
{
	...
	for (size_t i = 0; i < sizeof(cyhal_sdio_pads) / sizeof(cyhal_sdio_pads[0]); i++) {
		const struct cyhal_sdio_pad *s = &cyhal_sdio_pads[i];
		ret = cyhal_utils_set_iomux(s->iomux, s->mux_setting);
		if (ret < 0) {
		...
		/* 100K Ohm Pull up */
		char pus = s->doPullUp ? 2 : 0;
		char pue = s->doPullUp ? 1 : 0;
		char pke = s->doPullUp ? 1 : 0;
		ret = cyhal_utils_set_iopad(s->iopad, 0, pus, pue, pke, 0, 2, 1, 0);
		if (ret < 0) {
			cy_log_msg(CYLF_SDIO, CY_LOG_ERR, "can't configure SDIO pins iopad\n");
			return ret;
		}

		ret = cyhal_utils_set_iosel(s->isel, s->daisy);
		if (ret < 0) {
