...
static const whd_event_num_t auth_events[] = { WLC_E_EXT_AUTH_REQ, WLC_E_EXT_AUTH_FRAME_RX, WLC_E_NONE };
		...
		case WHD_AP_ROLE:
			...
			chan_switch->chspec =
					(wl_chanspec_t)(GET_C_VAR(whd_driver,
											CHANSPEC_BW_20) |
							GET_C_VAR(whd_driver, CHANSPEC_CTL_SB_NONE) | channel);
...
whd_result_t whd_wifi_set_pmksa(whd_interface_t ifp, const pmkid_t *pmkid)
{
	...
	CHECK_IOCTL_BUFFER(whd_proto_get_iovar_buffer(whd_driver, &buffer,
			sizeof(uint32_t) + MAXPMKID * sizeof(pmkid_t), IOVAR_STR_PMKID_INFO));
...
whd_result_t whd_wifi_get_ap_client_rssi(whd_interface_t ifp, int32_t *rssi, const whd_mac_t *client_mac)
{
	...
	client_rssi =
			(client_rssi_t *)whd_proto_get_ioctl_buffer(whd_driver, &buffer, ROUND_UP(sizeof(client_rssi_t), sizeof(uint32_t)));
		...
		cy_semaphore_t *semaphore)
{
	...
	if (whd_driver->chip_info.fwcap_flags & (1 << WHD_FWCAP_GCMP)) {
		if (auth_type == WHD_SECURITY_WPA3_ENT || auth_type == WHD_SECURITY_WPA3_192BIT_ENT) {
			...
			WPRINT_WHD_DEBUG(("set_wsec_info algos (0x%x) mask (0x%x)\n", algos, mask));
...

{
	...
	record->bss_type =
			...
			((bss_info->capability & DOT11_CAP_IBSS) !=
					0) ?
			WHD_BSS_TYPE_ADHOC :
		...
		uint32_t *count)
{
	...
	whd_scan_result_callback_t handler = (*count == 0) ? whd_scan_count_handler : whd_scan_result_handler;
...
whd_result_t whd_wifi_register_multicast_address(whd_interface_t ifp, const whd_mac_t *mac)
{
	...
	CHECK_IOCTL_BUFFER(whd_proto_get_iovar_buffer(whd_driver, &buffer,
			sizeof(uint32_t) + MAX_SUPPORTED_MCAST_ENTRIES * sizeof(whd_mac_t), IOVAR_STR_MCAST_LIST));
	CHECK_RETURN(whd_proto_get_iovar(ifp, buffer, &response));
...
#define IF_IS_STR(var, is) ((var) == is) ? #is 
uint32_t whd_wifi_is_ready_to_transceive(whd_interface_t ifp)
{
	...
	printf("\nROLE IS %s (%d)\n\n", 
		IF_IS_STR(ifp->role, WHD_AP_ROLE) : 
		IF_IS_STR(ifp->role, WHD_STA_ROLE): 
		IF_IS_STR(ifp->role, WHD_P2P_ROLE) :
		IF_IS_STR(ifp->role, WHD_INVALID_ROLE) :
		"unknown",
		ifp->role
	);
	return WHD_SUCCESS;
...
uint32_t whd_wifi_he_omi(whd_interface_t ifp, whd_he_omi_params_t *he_omi_params)
{
	...
	memset((uint8_t *)&he_omi, 0x00, sizeof(he_omi));
...
whd_result_t whd_arp_hostip_list_add(whd_interface_t ifp, uint32_t *host_ipv4_list, uint32_t count)
{
	...
	if (host_ipv4_list == NULL) {
		WPRINT_WHD_ERROR(("%s() BAD ARGS ifp:%p host_ipv4_list:%p count %u\n", __func__, ifp, host_ipv4_list, count));
		return WHD_BADARG;
	}
	/* check if unique */
	...
	if ((whd_ret == WHD_SUCCESS) && (filled > 0)) {
		...
		for (curr_index = 0; curr_index < filled; curr_index++) {
			for (new_index = 0; new_index < count; new_index++) {
				WPRINT_WHD_DEBUG(("%s() curr:%d of %d curr:0x%x new:%d of %d:0x%x\n", __func__, curr_index,
				...
				if (current_ipv4_list[curr_index] == host_ipv4_list[new_index]) {
					...
					if (new_index < count) {
						/* copy next one down */
						WPRINT_WHD_DEBUG(("move %d (+1) of %d \n", new_index, count));
...
whd_result_t whd_arp_hostip_list_clear_id(whd_interface_t ifp, uint32_t ipv4_addr)
{
	...
	if ((whd_ret == WHD_SUCCESS) && (filled > 0)) {
		...
		for (index = 0; index < filled; index++) {
			WPRINT_WHD_DEBUG(("%d %s() drop() 0x%x == 0x%x ? %s\n", __LINE__, __func__, host_ipv4_list[index], ipv4_addr, (host_ipv4_list[index] == ipv4_addr) ? "DROP" : ""));
		...
		for (i = 0; i < filter_list->num; i++) {
			...
			filter_ptr =
					(wl_pkt_filter_t *)((char *)filter_ptr +
							(WL_PKT_FILTER_FIXED_LEN + WL_PKT_FILTER_PATTERN_FIXED_LEN + 2 * in_filter->u.pattern.size_bytes));
...
whd_tko_param(whd_interface_t ifp, whd_tko_retry_t *whd_retry, uint8_t set)
{
	...
	if (set) {
		...
		wl_param_p->interval = whd_retry->tko_interval ==
						0 ?
				TCP_KEEPALIVE_OFFLOAD_INTERVAL_SEC :
				whd_retry->tko_interval;
		wl_param_p->retry_count = whd_retry->tko_retry_count ==
						0 ?
				TCP_KEEPALIVE_OFFLOAD_RETRY_COUNT :
				whd_retry->tko_retry_count;
		wl_param_p->retry_interval = whd_retry->tko_retry_interval ==
						0 ?
				TCP_KEEPALIVE_OFFLOAD_RETRY_INTERVAL_SEC :
				whd_retry->tko_retry_interval;
