...
uint32_t whd_wifi_get_ap_client_rssi(whd_interface_t ifp, int32_t *rssi, const whd_mac_t *client_mac)
{
	...
	client_rssi =
		(client_rssi_t *)whd_cdc_get_ioctl_buffer(whd_driver, &buffer, ROUND_UP(sizeof(client_rssi_t), sizeof(uint32_t)));
		...
		if (ap->channel) {
			...
			ext_join_params->assoc_params.chanspec_list[0] =
				(wl_chanspec_t)htod16((ap->channel |
					GET_C_VAR(whd_driver, CHANSPEC_BW_20) | GET_C_VAR(whd_driver, CHANSPEC_CTL_SB_NONE)));
	...
	void *handler_user_data)
{
	...
	record->band =
		((chanspec &
			 GET_C_VAR(whd_driver,
				 CHANSPEC_BAND_MASK)) ==
					GET_C_VAR(whd_driver, CHANSPEC_BAND_2G) ?
