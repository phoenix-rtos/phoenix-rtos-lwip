...
#else
	result = whd_bus_mem_bytes(whd_driver, BUS_READ, TRANS_ADDR(shared_addr), sizeof(internal_info->sh), (uint8_t *)&internal_info->sh);
#endif
...
whd_result_t whd_wifi_read_wlan_log(whd_driver_t whd_driver, char *buffer, uint32_t buffer_size)
{
	...
	if (wlan_chip_id == 43362) {
		return whd_wifi_read_wlan_log_unsafe(whd_driver, ((GET_C_VAR(whd_driver, CHIP_RAM_SIZE) + PLATFORM_WLAN_RAM_BASE) - 4), buffer, buffer_size);
	}
		...
		result = whd_wifi_read_wlan_log_unsafe(whd_driver, ((GET_C_VAR(whd_driver, CHIP_RAM_SIZE) + GET_C_VAR(whd_driver, ATCM_RAM_BASE_ADDRESS)) - 4), buffer,
				buffer_size);
...
static whd_bool_t whd_is_fw_sr_capable(whd_driver_t whd_driver)
{
	...
	if ((wlan_chip_id == 43430) || (wlan_chip_id == 43439)) {
		...
		/* check if fw initialized sr engine */
		result =
				whd_bus_read_backplane_value(whd_driver, (uint32_t)RETENTION_CTL(whd_driver), (uint8_t)sizeof(retention_ctl),
						(uint8_t *)&retention_ctl);
		if (result != WHD_SUCCESS) {
			return WHD_FALSE;
		}
		...
		uint32_t sdio_control_addr)
{
	...
	if (wlan_chip_id == 43012) {
		...
		CHECK_RETURN(whd_bus_read_backplane_value(whd_driver, D11SHM_ADDR(M_DS1_CTRL_SDIO), (uint8_t)2, (uint8_t *)&val));
