...
return_with_error:
	whd_assert("Error accessing backplane", 0 != 0);
	return 0;
}
...
whd_result_t whd_bus_spi_init(whd_driver_t whd_driver)
{
	...
	do {
		...
		*gspi_header =
				(whd_bus_gspi_header_t)SWAP32_16BIT_PARTS(SWAP32((uint32_t)((whd_bus_gspi_command_mapping[(int)BUS_READ] & 0x1) << 31) |
...
static whd_result_t whd_spi_download_firmware(whd_driver_t whd_driver)
{
	...
	CHECK_RETURN(whd_bus_spi_write_backplane_value(whd_driver, SDIO_INT_HOST_MASK(whd_driver), (uint8_t)4, I_HMB_SW_MASK));
...
void whd_bus_spi_init_stats(whd_driver_t whd_driver)
{
	...
	if (btdev && btdev->intr) {
		...
		res = whd_bus_spi_write_backplane_value(whd_driver, SDIO_INT_HOST_MASK(whd_driver), (uint8_t)4, I_HMB_FC_CHANGE);
		if (res != WHD_SUCCESS) {
