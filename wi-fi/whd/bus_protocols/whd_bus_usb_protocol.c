...
static whd_result_t whd_bus_usb_wait_and_init_device(whd_driver_t whd_driver)
{
    ...
    result = cyhal_usb_wait_state(whd_bus_usb_common.config.path, true, 5000 /* ms */);
    if (result == CY_RTOS_TIMEOUT) {
        WPRINT_WHD_ERROR(("Timed out waiting for USB to become available\n"));
        return WHD_TIMEOUT;
    }
...
static whd_result_t whd_bus_usb_init_internal(whd_driver_t whd_driver)
{
    ...
    if (whd_bus_usb_dl_needed(whd_driver)) {
        ...
        if (result != WHD_SUCCESS) {
            ...
            cyhal_usb_free(whd_driver->bus_priv->usb_obj);
            return result;
        }

        /* re-initialize USB as it's re-enumerated */
        rslt = cyhal_usb_wait_state(whd_bus_usb_common.config.path, false, 5000 /* ms */);
        if (rslt == CY_RTOS_TIMEOUT) {
            WPRINT_WHD_ERROR(("Timed out waiting for USB to go down\n"));
            cyhal_usb_free(whd_driver->bus_priv->usb_obj);
            return WHD_TIMEOUT;
        }
...
static inline whd_result_t whd_bus_usb_handle_rx(whd_driver_t whd_driver, whd_buffer_t buffer)
{
    ...
    if (result == CY_RSLT_SUCCESS) {
        ...
        return whd_bus_usb_rx_queue_enqueue(&buffer);
    }
    else if ((result & CY_RSLT_CODE_MASK) == ENOENT) {
        return WHD_BUS_FAIL;
    }
    else {
...
static void whd_bus_usb_rx_thread(cy_thread_arg_t arg)
{
    ...
    for (;;) {
        ...
        if (!whd_bus_rx_queue_is_full()) {
            ...
            result = whd_bus_usb_handle_rx(whd_driver, buffer);
            if (result != WHD_SUCCESS) {
            ...
            WPRINT_WHD_DEBUG(("whd queue is full, going to sleep\n"));
...
static whd_result_t whd_bus_usb_start_rx(whd_driver_t whd_driver)
{
    if (whd_bus_usb_common.rx.thread != 0) {
        WPRINT_WHD_INFO(("USB Bus: RX thread already running\n"));
        return WHD_SUCCESS;
    }
    ...
    if (result != CY_RSLT_SUCCESS) {
        ...
        cy_rtos_deinit_semaphore(&whd_bus_usb_common.rx.semaphore);
        return WHD_RTOS_ERROR;
    }
    ...
    if (result != CY_RSLT_SUCCESS) {
        ...
        cy_rtos_deinit_semaphore(&whd_bus_usb_common.rx.semaphore);
        cy_rtos_queue_deinit(&whd_bus_usb_common.rx.queue);
        return WHD_RTOS_ERROR;
    }
...
static void whd_bus_usb_rx_queue_buffers_release(whd_driver_t whd_driver)
{
    whd_buffer_t buffer;
    whd_result_t result = whd_bus_usb_rx_queue_dequeue(&buffer);
    while (result == WHD_SUCCESS) {
        whd_buffer_release(whd_driver, buffer, WHD_NETWORK_RX);
        result = whd_bus_usb_rx_queue_dequeue(&buffer);
    }
}
...
static bool whd_bus_usb_dl_needed(whd_driver_t whd_driver)
{
    ...
    if (id.chip == WHD_USB_POSTBOOT_ID)
    {
        ...
        whd_bus_usb_common.chipid = whd_chip_get_chip_id(whd_driver);
...
static whd_result_t whd_bus_usb_download_firmware(whd_driver_t whd_driver)
{
    ...
    while (state.bytes != image_size)
    {
        ...
        if ((state.bytes == sent) && (state.bytes != image_size))
        {
            ...
            if (size == 0) {
                /* nothing to do, we've reached the end */
                break;
            }
...
whd_result_t whd_bus_usb_send_buffer(whd_driver_t whd_driver, whd_buffer_t buffer)
{
    ...
    switch (sdpcm_header->sw_header.channel_and_flags & 0x0f)
    {
        ...
        case CONTROL_HEADER:  /* Sent IOCTL/IOVAR packet (CDC packet) */
        {
            ...
            if (whd_bus_usb_send_ctrl(whd_driver, cdc_header, &cdc_size) != WHD_SUCCESS) {
                break;
            }

            /* Receive control response */
            if (whd_bus_usb_receive_ctrl_buffer(whd_driver, &rec_buffer) != WHD_SUCCESS) {
                break;
            }
