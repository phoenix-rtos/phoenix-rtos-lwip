...
{
	return whd_bus_usb_attach(cybsp_get_wifi_driver(), NULL);
}
...
{
	if (init_usb(whd_driver, true) != WHD_SUCCESS) {
		WPRINT_WHD_ERROR((__FILE__ ":%d USB init failed\n", __LINE__));
...
static whd_result_t whd_bus_usb_wait_for_wlan_event(whd_driver_t whd_driver, cy_semaphore_t *transceive_semaphore)
{
	...
	if (whd_bus_usb_packet_available_to_read(whd_driver)) {
		whd_thread_notify(whd_driver);
		return WHD_SUCCESS;
	}

	return cy_rtos_get_semaphore(transceive_semaphore, CY_RTOS_NEVER_TIMEOUT, WHD_FALSE);
}
