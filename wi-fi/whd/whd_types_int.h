...
#define CHECK_IOCTL_BUFFER(buff) \
	if (buff == \
			NULL) { \
		WPRINT_WHD_ERROR_RETURN(("Buffer alloc failed in function %s at line %d \n", \
				__func__, __LINE__)); \
...
#define CHECK_PACKET_NULL(buff, err) \
	if (buff == \
			NULL) { \
		WPRINT_WHD_ERROR_RETURN(("No register function pointer in %s at line %d \n", \
				__func__, __LINE__)); \
		return err; \
	}
...
#define CHECK_RETURN(expr) \
	{ \
		...
		if (check_res != WHD_SUCCESS) { \
			WPRINT_WHD_ERROR_RETURN(("Function %s failed at line %d : code=%u type=%d module=%d \n", \
					__func__, __LINE__, \
					CY_RSLT_GET_CODE((unsigned int)check_res), CY_RSLT_GET_TYPE((unsigned int)check_res), CY_RSLT_GET_MODULE((unsigned int)check_res))); \
			return check_res; \
		} \
	}

#define CHECK_RETURN_UNSUPPORTED_OK(expr) \
	{ \
		whd_result_t check_res = (expr); \
		if (check_res != WHD_SUCCESS) { \
			return check_res; \
...
#define CHECK_RETURN_UNSUPPORTED_CONTINUE(expr) \
	{ \
		whd_result_t check_res = (expr); \
...
#define RETURN_WITH_ASSERT(expr) \
	{ \
		whd_result_t check_res = (expr); \
...
#define CHECK_RETURN_IGNORE(expr) \
	{ \
		whd_result_t check_res = (expr); \
		if (check_res != WHD_SUCCESS) { \
		} \
