...
static bool wifi_sta_is_connected(whd_ssid_t *ssid)
{
	...
	if (ssid != NULL) {
		...
		ssid->length = ap_info.SSID_len;
	}
	return true;
}


static bool wifi_is_idle(struct wifi_device *dev)
{
	if (dev == &wifi_common.ap) {
		return wifi_ap_is_idle();
	}
	else if (dev == &wifi_common.sta) {
		return !wifi_sta_is_connected(NULL);
	}
	else {
		return true;
	}
...
static void wifi_main_loop(struct wifi_device *dev)
{
	for (;;) {
		...
		mutexLock(dev->lock);
		condWait(dev->cond, dev->lock, cond_timeout * 1000 * 1000);
...
static void wifi_sta_thread(void *arg)
{
	...
	scan_timeout = STA_SCAN_TIMEOUT_SECONDS;
...
static int wifi_dev_start(struct wifi_device *dev)
{
	...
	if (sys_thread_opt_new(thread_name, thread, dev, WIFI_THREAD_STACKSZ, WIFI_THREAD_PRIO, &dev->tid) < 0) {
		mutexUnlock(dev->lock);
...
static int wifi_dev_write(id_t id, const char *data, size_t size)
{
	...
	else if (id == STA_DEV_ID && size == (sizeof("connect") - 1) && strncmp("connect", data, size) == 0) {
		wifi_dev_start(&wifi_common.sta);
	}
	else if (id == STA_DEV_ID && size == (sizeof("disconnect") - 1) && strncmp("disconnect", data, size) == 0) {
		wifi_dev_stop(&wifi_common.sta);
	}
	else if (id == AP_DEV_ID && size == (sizeof("start") - 1) && strncmp("start", data, size) == 0) {
	...
	else if (id == AP_DEV_ID && size == (sizeof("stop") - 1) && strncmp("stop", data, size) == 0) {
...
static int wifi_handle_ctrl(unsigned int port, msg_t *msg)
{
	switch (msg->type) {
		case mtOpen: {
			...
			int bytes = snprintf(wifi_common.ctrl.buf, sizeof(wifi_common.ctrl.buf), "wifi is %s", wifi_common.initialized ? "on" : "off");
