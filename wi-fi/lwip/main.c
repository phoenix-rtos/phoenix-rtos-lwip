...
static void wifi_main_loop(struct wifi_device *dev)
{
	for (;;) {
		...
		mutexLock(dev->lock);
		condWait(dev->cond, dev->lock, condTimeout * 1000 * 1000);
...
static inline time_t now_seconds(void)
{
	struct timespec tp;
	if (clock_gettime(CLOCK_MONOTONIC, &tp) < 0) {
		/* value of 0 should not be possible to get otherwise */
		return 0;
	}
	return tp.tv_sec + (tp.tv_nsec / (1000ULL * 1000ULL * 1000ULL));
}
...
static void wifi_sta_thread(void *arg)
{
	...
	remTimeout = STA_SCAN_TIMEOUT_SECONDS;

	do {
		if (wifi_sta_is_connected(&connectedTo)) {
		...
		now = now_seconds();
		if (now == 0) {
			wm_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "couldn't get current time");
			break;
		}
		...
		do {
			...
			condWait(wifi_common.sta.cond, wifi_common.sta.lock, remTimeout * 1000000ULL / STA_N_SCANS);
		...
		result = whd_wifi_join(wifi_common.sta.iface.whd_iface, &ssid, scan_result.security, key, keyLen);
		if (result != WHD_SUCCESS) {
